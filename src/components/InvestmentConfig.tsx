...
import { Eye, EyeOff, Save, Calculator, DollarSign, Percent, TrendingUp, Plus, Edit, Trash2, Target } from "lucide-react";
...
const InvestmentConfig = ({ 
  ...
  const SECRET_PASSWORD = "investment2024";
  ...
  const handlePasswordSubmit = (e: React.FormEvent) => {
    ...
    if (password === SECRET_PASSWORD) {
      ...
      toast({
        ...
        description: "You can now configure your investment plan.",
      });
    } else {
      toast({
        title: "Access Denied",
  ...
  const calculatePortfolioAmount = (portfolio: Portfolio) => {
    if (portfolio.allocationType === 'percentage') {
      return Math.round(totalInvestmentAmount * portfolio.allocationValue / 100);
    }
    return portfolio.allocationValue;
  };

  const calculateCategoryAmount = (category: PortfolioCategory, portfolioAmount: number) => {
  ...
  const calculateFundAmount = (fund: Fund, categoryAmount: number, totalFunds: number) => {
  ...
  const updatePortfolioAmounts = (portfolios: Portfolio[]) => {
    return portfolios.map(portfolio => {
      ...
      const updatedCategories = portfolio.categories.map(category => {
        const categoryAmount = calculateCategoryAmount(category, portfolioAmount);
        const updatedFunds = category.funds.map(fund => ({
          ...fund,
          allocatedAmount: calculateFundAmount(fund, categoryAmount, category.funds.length)
        }));
        return {
          ...category,
  ...
  const addPortfolio = (name: string, allocationType: 'percentage' | 'amount', allocationValue: number) => {
  ...
  const deletePortfolio = (portfolioId: string) => {
    const updatedPortfolios = investmentPlan.portfolios.filter(p => p.id !== portfolioId);
    const recalculatedPortfolios = updatePortfolioAmounts(updatedPortfolios);
    const newPlan = { portfolios: recalculatedPortfolios };
    setInvestmentPlan(newPlan);
  };

  const addCategory = (portfolioId: string, name: string, allocationType: 'percentage' | 'amount', allocationValue: number) => {
  ...
  const updateCategory = (portfolioId: string, categoryId: string, name: string, allocationType: 'percentage' | 'amount', allocationValue: number) => {
    const updatedPortfolios = investmentPlan.portfolios.map(portfolio =>
      portfolio.id === portfolioId
        ? {
            ...portfolio,
  ...
  const updateFund = (portfolioId: string, categoryId: string, fundId: string, name: string) => {
    const updatedPortfolios = investmentPlan.portfolios.map(portfolio =>
      portfolio.id === portfolioId
        ? {
            ...portfolio,
  ...
  const deleteFund = (portfolioId: string, categoryId: string, fundId: string) => {
    const updatedPortfolios = investmentPlan.portfolios.map(portfolio =>
      portfolio.id === portfolioId
        ? {
            ...portfolio,
            categories: portfolio.categories.map(category =>
              category.id === categoryId
                ? { ...category, funds: category.funds.filter(f => f.id !== fundId) }
                : category
            )
          }
        : portfolio
  ...
  const getTotalAllocatedAmount = () => {
    return investmentPlan.portfolios.reduce((sum, portfolio) => sum + portfolio.allocatedAmount, 0);
  };

  const getTotalAllocatedPercentage = () => {
    return totalInvestmentAmount > 0 ? (getTotalAllocatedAmount() / totalInvestmentAmount) * 100 : 0;
  };
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogTrigger asChild>
          <Button variant={portfolio ? "ghost" : "default"} size="sm">
            {portfolio ? <Edit className="h-4 w-4" /> : <><Plus className="h-4 w-4 mr-2" />Add Portfolio</>}
          </Button>
        </DialogTrigger>
        <DialogContent>
          <DialogHeader>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex justify-end space-x-2">
              ...
              <Button type="submit">{portfolio ? 'Update' : 'Add'} Portfolio</Button>
            </div>
          </form>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogTrigger asChild>
          <Button variant={category ? "ghost" : "outline"} size="sm">
            {category ? <Edit className="h-4 w-4" /> : <><Plus className="h-4 w-4 mr-2" />Add Category</>}
          </Button>
        </DialogTrigger>
        <DialogContent>
          <DialogHeader>
  ...
  const FundDialog = ({ portfolioId, categoryId, fund, onSave }: { portfolioId: string; categoryId: string; fund?: Fund; onSave: (name: string) => void }) => {
    const [open, setOpen] = useState(false);
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogTrigger asChild>
          <Button variant={fund ? "ghost" : "outline"} size="sm">
            {fund ? <Edit className="h-4 w-4" /> : <><Plus className="h-4 w-4 mr-2" />Add Fund</>}
          </Button>
        </DialogTrigger>
        <DialogContent>
          <DialogHeader>
        ...
        <Card className="shadow-card">
          <CardHeader className="text-center">
            ...
            <p className="text-sm text-muted-foreground">
              Enter password to access investment plan configuration
            </p>
          </CardHeader>
          <CardContent>
            ...
            <div className="mt-4 p-3 bg-muted/50 rounded-lg">
              <p className="text-xs text-muted-foreground text-center">
                Demo password: <code className="bg-background px-1 rounded">investment2024</code>
              </p>
            </div>
          </CardContent>
        </Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
            <div className="text-center p-4 bg-background/50 rounded-lg">
              <p className="text-sm text-muted-foreground">Total Investment Budget</p>
              <p className="text-2xl font-bold text-primary">₹{totalInvestmentAmount.toLocaleString()}</p>
            </div>
            <div className="text-center p-4 bg-background/50 rounded-lg">
              <p className="text-sm text-muted-foreground">Total Allocated</p>
              <p className="text-2xl font-bold text-secondary">₹{getTotalAllocatedAmount().toLocaleString()}</p>
              <p className="text-xs text-muted-foreground">{getTotalAllocatedPercentage().toFixed(1)}%</p>
            </div>
            <div className="text-center p-4 bg-background/50 rounded-lg">
              <p className="text-sm text-muted-foreground">Remaining</p>
              <p className={`text-2xl font-bold ${
                (totalInvestmentAmount - getTotalAllocatedAmount()) >= 0 ? 'text-success' : 'text-destructive'
              }`}>
                ₹{(totalInvestmentAmount - getTotalAllocatedAmount()).toLocaleString()}
              </p>
            </div>
          </div>
      ...
      <Card className="shadow-card">
        <CardHeader>
          <CardTitle className="flex items-center justify-between">
            ...
            <PortfolioDialog onSave={(name, allocationType, allocationValue) => addPortfolio(name, allocationType, allocationValue)} />
          </CardTitle>
        </CardHeader>
                  ...
                  <CardTitle className="flex items-center justify-between">
                    <div>
                      ...
                      <div className="flex items-center gap-4 text-sm text-muted-foreground">
                        ...
                        <span>•</span>
                        <span>₹{portfolio.allocatedAmount.toLocaleString()}</span>
                      </div>
                    </div>
                        ...
                        <AlertDialogContent>
                          <AlertDialogHeader>
                            ...
                            <AlertDialogDescription>
                              Are you sure you want to delete "{portfolio.name}"? This will also delete all categories and funds within this portfolio.
                            </AlertDialogDescription>
                          </AlertDialogHeader>
                          <AlertDialogFooter>
                  ...
                  {!portfolio.enableCategories ? (
                    ...
                    <div className="space-y-4">
                      {portfolio.categories.map((category) => (
                        <Card key={category.id} className="border-l-4 border-l-secondary">
                            ...
                            <CardTitle className="flex items-center justify-between text-base">
                              ...
                              <div className="flex items-center space-x-2">
                                <CategoryDialog 
                                  portfolioId={portfolio.id}
                                  category={category} 
                                  onSave={(name, allocationType, allocationValue) => updateCategory(portfolio.id, category.id, name, allocationType, allocationValue)} 
                                />
                                <AlertDialog>
                                  <AlertDialogTrigger asChild>
                                    <Button variant="ghost" size="sm">
                                  ...
                                  <AlertDialogContent>
                                    <AlertDialogHeader>
                                      ...
                                      <AlertDialogDescription>
                                        Are you sure you want to delete "{category.name}"? This will also delete all funds within this category.
                                      </AlertDialogDescription>
                                    </AlertDialogHeader>
                                    <AlertDialogFooter>
                                      <AlertDialogCancel>Cancel</AlertDialogCancel>
                                      <AlertDialogAction
                                        onClick={() => deleteCategory(portfolio.id, category.id)}
                                        className="bg-destructive text-destructive-foreground hover:bg-destructive/90"
                                      >
                                        Delete
                                      </AlertDialogAction>
                                  ...
                                  <div key={fund.id} className="flex items-center justify-between p-3 bg-muted/30 rounded-lg">
                                    <div>
                                      ...
                                      <p className="text-xs text-muted-foreground">
                                        ...
                                        {category.funds.length > 0 && ` (${(fund.allocatedAmount / category.allocatedAmount * 100).toFixed(1)}%)`}
                                      </p>
                                    </div>
                                    <div className="flex items-center space-x-2">
                                      {portfolio.enableFunds && (
                                      ...
                                      <AlertDialog>
                                        ...
                                        <AlertDialogContent>
                                          ...
                                          <AlertDialogFooter>
                                            <AlertDialogCancel>Cancel</AlertDialogCancel>
                                            <AlertDialogAction
                                              onClick={() => deleteFund(portfolio.id, category.id, fund.id)}
                                              className="bg-destructive text-destructive-foreground hover:bg-destructive/90"
                                            >
                                              Delete
                                            </AlertDialogAction>
